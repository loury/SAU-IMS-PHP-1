...
abstract class BaseAdminTask extends BaseTask
{
    ...
    public function getTaskById($tid){
        $sql1 = "select n.id `id`,`title`,`time`,c.name `name`,`text`
                from `notice` n
                join `clubinfo` c on n.club_id = c.club_id
                where n.id = ? ";//获得任务的信息
        $sql2 = "select u.name `recipients` 
                from user_task ut 
                join userinfo ui on ut.user_id = ui.
                where ut.id = ?
        ...
        try{
            ...
            $stmt1 -> bindParam(1,$nid,PDO::PARAM_INT);//公告id
            $stmt2 -> bindParam(1,$nid,PDO::PARAM_INT);//公告id
            if(!$stmt1 -> execute()){
                return false;//失败返回false
            }
            if(!$stmt2 -> execute()){
